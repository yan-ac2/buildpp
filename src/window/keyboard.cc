...
namespace keyboard {
    ...
    // KeyState
    constexpr void KeyState::setRawDown(bool down) noexcept {
        if (down) kState |= rawKeyDown;
        else      kState &= ~rawKeyDown;
    }
    
    constexpr void KeyState::setToggle(bool totalState) noexcept {
        if (totalState) kState |= kIsToggled;
        else            kState &= ~kIsToggled;
    }
    
    constexpr void KeyState::update() noexcept {
    ...
    // Basic state getters
    constexpr bool KeyState::IsDown()      const noexcept { return (kState & kIsDown) != 0; }
    constexpr bool KeyState::WasDown()     const noexcept { return (kState & kWasDown) != 0; }
    constexpr bool KeyState::ChangedDown() const noexcept { return (kState & kChangedDown) != 0; }
    constexpr bool KeyState::IsToggled()   const noexcept { return (kState & kIsToggled) != 0; }
    constexpr bool KeyState::RawKeyDown()  const noexcept { return (kState & rawKeyDown) != 0; }
    // --- Action Helpers ---
    constexpr KeyState::operator bool() const noexcept { return (kState & kIsDown) != 0; }
    // True only on the exact frame the key transitions from UP to DOWN
    constexpr bool KeyState::justPressed() const noexcept { 
        return (kState & (kIsDown | kWasDown)) == kIsDown; 
    }
    
    // True only on the exact frame the key transitions from DOWN to UP
    constexpr bool KeyState::justReleased() const noexcept { 
    ...
    //  Keyboard
    //
    constexpr bool Keyboard::poll(int32 tickMS, int32 dtimeMS) noexcept {
    ...
    constexpr KeyState& Keyboard::operator[](Key key) {
        using underlying = std::underlying_type_t<decltype(key)>;
        return KeysState[static_cast<underlying>(toPlatform(key))];
    }
