...
struct outPath {
    ...
    void setRootPath(std::string_view root) { rootPath = root;}
    void setExePath(std::string_view exe)   { exePath = rootPath / exe;}
...
class cProject{
    ...
    cProject(outPath* path, projectType exe) {
        projectOutPath = path;
        outFile = exe;
        std::cout << "Project initialized at " << this->path.c_str() << std::endl;
    };
    ...
    void link(std::string_view target) {
        const std::string Executable {fmt(" -o ", fmt((projectOutPath->exePath / target).string(), platform), " ")};
        std::string cmd; 
        std::string Object     {fmt(" -fprebuilt-module-path=", (projectOutPath->modulePath / ".").string(), " " ,
                                (projectOutPath->objPath / target).string(), ".o ")};
    ...
    void compileC(std::string_view inPath)
    {
        ...
        const std::string obj {fmt((projectOutPath->objPath / f_path.stem()).string(), ".o")};
        ...
        std::string cmd {fmt(Compiler, f_found ? compileInclude : "" ,fmt(" -c ", inPath, " -o ", obj))};
...
class Project
{
    ...
    std::string Main            {};
    std::string Options         {"-std=c++23 -stdlib=libc++ "};
    std::string Compiler        {"clang++-20 "};
    std::string compileInclude  {};
    public:
    enum projectType : int {
    ...
    Project(outPath* path, projectType exe) {
        projectOutPath = path;
        outFile = exe;
        std::cout << "Project initialized at " << this->path.string() << std::endl;
    };

    void getLib(cProject* cProj)
    {
    ...
    void preCompile(std::string_view inpath) {
        ...
        f_srcInput.append (fmt(" -fprebuilt-module-path=",(projectOutPath->modulePath / ".").string()," --precompile ", f_path.string()," "));
    ...
    void link(std::string_view target) {
        std::string_view targetOut {fmt((projectOutPath->exePath / target).c_str())};
        std::string Executable {fmt(" -o ", fmt(targetOut, platform), " ")};
        std::string Object     {fmt(modules.empty() ? "" : fmt(" -fprebuilt-module-path=", (projectOutPath->modulePath / ".").c_str()), 
            " " )};
        if (fs::exists(fmt(targetOut, platform))) {
            fs::rename(fmt(targetOut, platform), fmt(targetOut, platform, ".old"));
        }
        for (auto& p : object) {
            for (const auto& deps : dependency)
            {
